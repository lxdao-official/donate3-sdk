...
export const getFasterIpfsLink = (options: {
  ipfs: string;
  timeout?: number;
  gateways?: string[];
}) => {
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    function fetchCallback(
      opts: { index: number; gateway: string },
      err?: Error,
      url?: string,
      response?: any,
    ) {
    ...
    const taskGatewayList =
      fasterGatewayCache && inGateways ? [fasterGatewayCache] : gateways;

    tasks = taskGatewayList.map((gateway, index) => {
      return convertIpfsToLink(
        { ipfs, gateway, timeout },
        (err, url, response) => {
          fetchCallback({ index, gateway }, err, url, response);
        },
      );
    });
  });
